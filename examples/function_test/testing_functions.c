...
{
    mt_uhf_errorcode_t ret = mt_uhf_set_inventory_mask(
        mt_uhf_mem_bank_EPC, 8, (const uint8_t *)"\x20\x91\x0D\xFA", 4 * 8 - 2);
...
bool test_region_mode(enum mt_uhf_region region, enum mt_uhf_rf_mode mode)
{
    ...
    if ((ret = mt_uhf_set_rf_mode(region, mode)) < mt_uhf_errorcode_success) {
        printf("Setting RF mode returned error %d: %s \n", ret, mt_uhf_error2string(ret));
        return false;
    }
    enum mt_uhf_region  _region;
    enum mt_uhf_rf_mode _mode;
    if ((ret = mt_uhf_get_rf_mode(&_region, &_mode)) < mt_uhf_errorcode_success) {
...
bool test_device_info(void)
{
    ...
    if (ret != mt_uhf_errorcode_success) {
        printf("Getting identification returned error %d: %s \n", ret, mt_uhf_error2string(ret));
        return false;
    }
    printf("Reader data:\nSW: %s ", id.fw_name);
    for (int i = 0; i < 4; i++)
...
{
    mt_uhf_errorcode_t ret;
    if ((ret = mt_uhf_set_session(mt_uhf_session_max + 1)) < mt_uhf_errorcode_success)
        printf("Setting invalid RF mode returned error %d: %s \n", ret, mt_uhf_error2string(ret));
    typeof(session) _session;
    if ((ret = mt_uhf_get_session(&_session)) < mt_uhf_errorcode_success) {
        printf("Getting invalid RF mode returned error %d: %s \n", ret, mt_uhf_error2string(ret));
        return false;
    }
    printf("Default session: %i\n", _session);
    if ((ret = mt_uhf_set_session(session)) < mt_uhf_errorcode_success) {
        printf("Setting invalid RF mode returned error %d: %s \n", ret, mt_uhf_error2string(ret));
        return false;
    }
    //Resets session local to unknown
    if (mt_uhf_errorcode_invalid_parameter != (ret = mt_uhf_set_session(mt_uhf_session_max + 1))) {
        printf("Setting invalid RF mode returned error %d: %s \n", ret, mt_uhf_error2string(ret));
        return false;
    }
    if ((ret = mt_uhf_get_session(&_session)) < mt_uhf_errorcode_success) {
...
bool test_mux_antenna(const uint8_t *antennas, unsigned antenna_count)
{
    ...
    if (!antenna_count) {
        //No antenna, expect error answer leading to range error
        ret = mt_uhf_get_multiplex_antennas(_multiplex, ARRAY_SIZE(_multiplex));
        if (ret == mt_uhf_errorcode_general_fault)
            return true;
...
static mt_uhf_errorcode_t _hbt_cb(const char *prefix, char *data, bool finalized)
{
    static uint32_t c = 0;
    if (data || !finalized || !prefix)
        return mt_uhf_errorcode_format_fault;
    if (strcmp(prefix, "+HBT"))
...
{
    mt_uhf_errorcode_t ret = mt_uhf_heartbeat_set(hbt_time, _hbt_cb);
    if (ret < mt_uhf_errorcode_success) {
    ...
    while (mt_rfid_reader_get_time() - start_test <= test_time && !abort_requested()) {
        if ((mt_rfid_reader_get_time() - hbt_test_last) > 10000) {
            printf("HBT timeout\n");
            ret = mt_uhf_heartbeat_set(0, NULL);
            break;
        }
                ...
                void (*round_done_cb)(struct mt_uhf_inventory_buffer *buf))
{
    ...
    while (mt_rfid_reader_get_time() - start_test <= test_time && !abort_requested()) {
        ...
        ret = mt_uhf_resolve();
        if (ret == mt_uhf_errorcode_busy_call_again || ret == mt_uhf_errorcode_no_data_available)
            continue;
        if (ret == mt_uhf_errorcode_already)
               ...
               void (*round_done_cb)(struct mt_uhf_inventory_buffer *buf))
{
    ...
    while (mt_rfid_reader_get_time() - start_test <= test_time && !abort_requested()) {
        ...
        if (ret == mt_uhf_errorcode_already)
            ...
            printf("Unexpected success answer on mt_uhf_resolve()\n");
            goto exit;
        } else
            printf("Resolve returned error %d: %s \n", ret, mt_uhf_error2string(ret));
    }
...
bool test_tid(void)
{
    ...
    mt_uhf_errorcode_t       ret = mt_uhf_inventory_read_tid(TIDs, ARRAY_SIZE(TIDs), 0);
    if (ret < mt_uhf_errorcode_success) {
...
bool test_rw_masked(struct mt_uhf_gen2_tag *tag)
{
    ...
    ret = mt_uhf_write_data(
        mt_uhf_mem_bank_USR, wdata, sizeof(wdata), 0, &epc, 1, NULL, &error_count, 0);
    if (ret < mt_uhf_errorcode_success || error_count) {
        printf(
            "Write error %d: %s with %u tag errors\n", ret, mt_uhf_error2string(ret), error_count);
        return false;
    }
    ...
    ret                         = mt_uhf_read_data(
        mt_uhf_mem_bank_USR, sizeof(rdata), 0, &answer, NULL, 1, &epc, &error_count, 0);
    if (ret < mt_uhf_errorcode_success || error_count) {
        printf(
            "Read error %d: %s with %u tag errors\n", ret, mt_uhf_error2string(ret), error_count);
        return false;
    }
...
bool test_outputs(void)
{
    ...
    mt_uhf_errorcode_t ret = mt_uhf_set_output(out);
    if (ret < mt_uhf_errorcode_success) {
...
bool test_inputs(mt_uhf_input_states_t *expected)
{
    ...
    mt_uhf_errorcode_t    ret = mt_uhf_get_input(data);
    if (ret < mt_uhf_errorcode_success) {
